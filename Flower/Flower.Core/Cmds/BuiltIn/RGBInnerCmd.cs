...
{
    public sealed class RgbInnerCmd : IFlowerCommand
    {
        public string Id => "rgb.inner";
        public string DisplayName => "Set RGB (Inner Ring)";
        ...
        public void ValidateArgs(FlowerCategory category, IReadOnlyDictionary<string, object?> args)
        {
            if (category != FlowerCategory.BigTulip)
                throw new NotSupportedException("RGBIN is only supported for BigTulip.");
            ...
            IReadOnlyDictionary<string, object?> args)
        {
            ...
            int g = (int)args["g"]!;
            int b = (int)args["b"]!;
            var frame = $"{flowerId}/RGBIN:{r},{g},{b}\n";
