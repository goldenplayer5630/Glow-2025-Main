...
namespace Flower.Core.Cmds.BuiltIn
{
    ...
    public sealed class RgbCmd : IFlowerCommand
    {
        public string Id => "rgb";
        public string DisplayName => "Set RGB (Both Rings)";
        ...
        public void ValidateArgs(FlowerCategory category, IReadOnlyDictionary<string, object?> args)
        {
            if (category != FlowerCategory.BigTulip)
                throw new NotSupportedException("RGB is only supported for BigTulip.");
